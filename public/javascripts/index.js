	...
	$.ajax({
		...
		$('#cards-container').fadeOut(500, function(){$('#cards-container').empty()}).fadeIn(500, function(){
			...
			for (var i=0; i<data.length; i++) {
				var card = $('<a class="card-container" href="tab/'+data[i]._id+'"><div class="card"><div class="heading '+data[i].rating_val+'"><h1>'+data[i].song+' '+data[i].type+'</h1><h2>'+data[i].artist+'</h2><span>'+data[i].rating_val+'<br>View Tab</span></div><div class="ratebar" data-value="'+data[i].rating_val+'"></div><div class="rating">'+data[i].rating_count+' reviews<h4>'+data[i].rating_text+'</h4></div></div></a>');
